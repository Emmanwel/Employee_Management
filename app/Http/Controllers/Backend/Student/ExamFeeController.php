...
class ExamFeeController extends Controller
{
    ...
    public function ExamFeeView()
    {
        ...
        return view('backend.students.exam_fee.exam_fee_view', $data);
    }
    ...
    public function ExamFeeClassData(Request $request)
    {
        ...
        foreach ($allStudent as $key => $v) {
            ...
            $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-' . $color . '" title="PaySlip" target="_blanks" href="' . route("student.exam.fee.slip") . '?class_id=' . $v->class_id . '&student_id=' . $v->student_id . '&exam_type_id=' . $request->exam_type_id . ' ">Exam Fee</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
    ...
    public function ExamFeePayslip(Request $request)
    {
        ...
        // $pdf = PDF::loadView('backend.students.exam_fee.exam_fee_pdf', $data);
        // $pdf->SetProtection(['copy', 'print'], '', 'pass');

        $pdf = app('dompdf.wrapper');

        $pdf->loadView('backend.students.exam_fee.exam_fee_pdf', $data);
